...
// 
//
// Related Topics 数组 字符串 模拟 👍 366 👎 0
...
public class TextJustification68 {
    ...
    public static void main(String[] args) {
        Solution solution = new TextJustification68().new Solution();
    ...
    public List<String> fullJustify(String[] words, int maxWidth) {
        ...
        for (int i = 0; i< words.length ; i++) {
            if (length + words[i].length() <= maxWidth) {
                ...
                if (temp.size() > 1) {
                    int a = (maxWidth+temp.size() - length) / (temp.size() - 1);
                    int b = (maxWidth+temp.size() - length) % (temp.size() - 1);
                    ...
                    for (int m = 0;m<maxWidth - length+1; m++) {
                        builder.append(' ');
                    }
                }
